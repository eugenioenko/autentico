...
const DefaultLogo: React.FC = () => (
  <svg fill="#ff7b00" width="28" height="28" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" transform="matrix(-1,0,0,1,0,0)">
    ...
    <header className="h-16 bg-theme-body flex items-center gap-4 px-5 md:px-8">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        {settings.theme_logo_url ? (
          <img src={settings.theme_logo_url} alt="Logo" className="h-7 w-7 object-contain" />
        ) : (
          <DefaultLogo />
        )}
        <span className="font-bold tracking-tight">{settings.theme_title}</span>
      </div>
      <h1 className="text-xl font-bold tracking-tight flex-1 text-theme-muted">{title}</h1>
      <ThemeSelector />
    </header>
  );
};
