...
If `private_key.pem` may be exposed:

1. **Generate a new RSA key**: `make generate-key` or `openssl genrsa -out private_key.pem 4096`
2. **Replace the key** at `AUTENTICO_PRIVATE_KEY_PATH` and restart Autentico
...
4. **Update `AUTENTICO_AUTH_JWK_CERT_KEY_ID`** if your clients cache the JWKS by key ID
