...
- **Headless admin-API tokens (CI/CD)**: By default, `autentico-admin` only supports `authorization_code` + `refresh_token`, which require a browser. To fetch admin-API tokens from scripts (e.g. integration tests), pass `--enable-admin-password-grant` on first `autentico onboard` or `autentico start` — including `start --auto-setup` — or set `AUTENTICO_ENABLE_ADMIN_PASSWORD_GRANT=true`. This seeds the admin client with the `password` (ROPC) grant, allowing `POST /oauth2/token` with `grant_type=password`. The flag only applies when the admin client is seeded for the first time; it does not modify an existing client. MFA and account lockout still apply; use a strong admin password and enable TOTP for this account in production.
