...
**Why:**
- Resource servers can verify tokens using only the public key (from `/.well-known/jwks.json`) — they never need access to the private key or to make a network call to Autentico per request
...
The RSA key is 4096-bit by default. A new key pair can be generated with `make generate-key`.
