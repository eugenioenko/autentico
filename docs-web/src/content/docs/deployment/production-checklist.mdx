...
- [ ] **Persistent RSA key** — `AUTENTICO_PRIVATE_KEY_PATH` points to a volume-mounted location. If it's ephemeral, tokens signed before a restart will fail validation.
- [ ] **Private key backup** — `private_key.pem` is backed up securely. Losing it invalidates all issued tokens.
