...
The `--auto-setup` flag generates a `.env` with fresh secrets on first run and reuses it on restarts. For a custom URL, pass it via `-e AUTENTICO_APP_URL=https://auth.example.com`.
...
| `autentico init --dev` | Disable the `Secure` flag on CSRF, IdP session, and refresh token cookies, and enable CORS. Use this for local HTTP development only — never in production. |
| `autentico start` | Start the server. Auto-applies pending migrations. Reads `.env` from the current directory, or from the path set in `AUTENTICO_ENV_FILE`. |
| `autentico start --auto-setup` | Generate a `.env` if one does not exist, then start. Safe to pass on every run. |
| `autentico start --no-auto-migrate` | Start without applying migrations automatically. The server will refuse to start if the database is behind. |
