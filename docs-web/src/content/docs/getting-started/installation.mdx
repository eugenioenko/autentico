...
<Aside type="caution" title="URL mismatch after restart">
...
| `autentico onboard --enable-admin-password-grant` | Also seed the `autentico-admin` client with the `password` (ROPC) grant so admin-API tokens can be fetched headlessly. Works on both `onboard` and `start` (including `start --auto-setup`), and via `AUTENTICO_ENABLE_ADMIN_PASSWORD_GRANT=true`. Only takes effect when seeding a fresh admin client — existing clients are left alone. |
