...
<p align="center">
  <img src="assets/autentico-demo.webp" alt="Auténtico first-run onboarding and admin UI">
</p>

<p align="center">
  <em>From an empty directory to a running identity provider.<br>
  Onboarding, self-signup, registering a client, 2FA enrollment, and the audit log.<br>
  <a href="assets/autentico-demo.mp4">Same walkthrough as an MP4 →</a></em>
</p>
...
To fetch admin-API tokens from scripts (integration tests, CI/CD), pass `--enable-admin-password-grant` on `autentico onboard` (or set `AUTENTICO_ENABLE_ADMIN_PASSWORD_GRANT=true`). This seeds the `autentico-admin` client with the `password` (ROPC) grant so `POST /oauth2/token` with `grant_type=password` can issue admin tokens headlessly. If you onboard via the browser wizard instead, add the `password` grant afterwards from the Admin UI (Clients → `autentico-admin`). MFA and account lockout still apply.
...
| `AUTENTICO_DB_FILE_PATH`         | SQLite database file path                                      | `./autentico.db`     |
...
Stress tests using [k6](https://k6.io) exercise the full PKCE auth code flow (authorize → login → token exchange → introspect → refresh). See the [performance benchmarks](#on-sqlite-as-the-database) for results and [`stress/README.md`](stress/README.md) for the full methodology, test profiles, and how to reproduce.
