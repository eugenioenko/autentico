...
K6_IMG="grafana/k6"
SCRIPTS="$(pwd)/stress"
TEST_SCRIPT=/scripts/load-30s-100vu-nosleep.js
RESULTS_FILE="$(pwd)/wal_vertical_30s_100vu.txt"
...
echo "WAL Mode Vertical Scaling (30s, 100 VU) — $(date)"
echo "WAL Mode Vertical Scaling (30s, 100 VU) — $(date)" >> "$RESULTS_FILE"
echo "" >> "$RESULTS_FILE"
